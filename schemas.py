...
class BaseCampaign(BaseModel):
    ...
    )  # Setting min value makes field required
...
class ResponseCampaign(BaseCampaign):
    ...
    )  # Allows SQLAlchemy to use dot notation when accessing dicts
...
class UpdateCampaign(BaseCampaign):
    ...
    )  # | None makes field optional BUT need a default value then
